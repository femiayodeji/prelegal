// Data layer for the Mutual NDA creator (PL-3).
//
// The Standard Terms below are the Common Paper Mutual NDA Standard Terms
// (Version 1.0), reproduced verbatim and free to use/modify under CC BY 4.0:
// https://commonpaper.com/standards/mutual-nda/1.0/
// They are fixed boilerplate; only the Cover Page fields are user-editable.
...
export interface Party {
  ...
  title: string;
  /** Email or postal address used for legal notices. */
...
/** All user-editable Cover Page values. */
...
/** Sensible defaults mirroring the Common Paper cover page sample values. */
...
/** A numbered clause of the Standard Terms. */
 ...
 */
export const standardTerms: StandardTerm[] = [
  {
...
/** Formats a year count with the correct singular/plural unit, e.g. "1 year". */
export function pluralizeYears(n: number): string {
  return `${n} ${n === 1 ? "year" : "years"}`;
}

/** Human-readable summary of the selected MNDA term, e.g. for the cover page. */
export function describeTerm(data: NdaData): string {
  if (data.termKind === "untilTerminated") {
    return "Continues until terminated in accordance with the terms of the MNDA.";
  }
  return `Expires ${pluralizeYears(data.termYears)} from the Effective Date.`;
}

/** Human-readable summary of the confidentiality period. */
export function describeConfidentiality(data: NdaData): string {
  if (data.confidentialityKind === "perpetuity") {
    return "In perpetuity.";
  }
  return `${pluralizeYears(data.confidentialityYears)} from the Effective Date, but in the case of trade secrets until the Confidential Information is no longer considered a trade secret under applicable laws.`;
}

/** Format an ISO date (yyyy-mm-dd) as a readable long date; falls back to a placeholder. */
